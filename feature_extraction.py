...
def small_feature_extraction_center(slide_name, file_path, output_path, feature_extractor, tile_dims = [224,224], check_filter = True,copytarget = False):
    if copytarget:
        copytarget = './tempslide{}'.format(slide_name)
        copyfile(file_path, copytarget)
        slide = openslide.OpenSlide(copytarget)
    ...
    if num_tiles_y > 100 and num_tiles_y > 100:
        ...
        for i_x in tqdm(range(100)):
            for i_y in tqdm(range(100)):
                curr_x = i_x*tile_dim_x
                curr_y = i_y*tile_dim_y
                tile_curr = read_whole[curr_x:curr_x+tile_dim_x, curr_y:curr_y + tile_dim_y, :]
                if check_filter:
                    if isBlackPatch_S(tile_curr, rgbThresh=20, percentage=0.05) or isWhitePatch_S(tile_curr, rgbThresh=220,
                                                                                              percentage=0.25):
                        continue
